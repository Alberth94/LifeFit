...
const ProfilePic = ({ currentUserName }) => {
  ...
  const handleAvatarUpload = async (e) => {
    ...
    const storageRef = ref(storage, `ProfilePic/${currentUserName}/${file.name}`);
    await uploadBytes(storageRef, file);
    const downloadUrl = await getDownloadURL(storageRef);
    const userCollectionRef = collection(db, "users");
    const userQueryRef = query(userCollectionRef, where("userName", "==", currentUserName));
  ...
  useEffect(() => {
    ...
    const postQueryRef = query(postCollectionRef, where("userName", "==", currentUserName));
